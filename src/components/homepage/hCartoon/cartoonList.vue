...
export default {
  ...
  methods: {
    ...
    send(index) {
      ...
      this.$bus.$emit("data", arr.name, arr.enname, arr.largeImg, arr.desc);
    },
  },
  beforeCreate() {
    //插件问题，需要在实例初始化之后就请求数据
    this.axios({
      method: "get",
      url: "/typeProducts",
      params: {
  ...
  mounted() {
    var ctnOneInterval = setInterval(() => {
      if (this.arr != "") {
        banner.ctnOne();
        clearInterval(ctnOneInterval)
      }
    }, 100);
    // var ctnOneIntervarl = setInterval(() => {
    //   // if (this.imageArr != "") {
    //   //   cartoon.cartoonList();
    //   //   clearInterval(ctnOneIntervarl);
    //   // }
