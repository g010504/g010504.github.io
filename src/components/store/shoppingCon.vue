  ...
  <!-- <div class="shop">
    <div class="row" v-for="(item, index) in shopArr" :key="index">
      <div class="row-box">
        ...
        <span class="num">月销: {{ item.num }}</span>
        <span class="iconfont icon-jiarugouwuche"></span>
      </div>
    </div>
  </div> -->
  <div class="shop">
    <div class="row" v-for="(item, index) in shopArr" :key="index" @click="getDetail(index)">
      <div class="row-box">
        <div class="img">
  ...
  methods: {
    getshopData() {
      this.axios({
        ...
        this.shopArr = res.data.result
        console.log(res.data.result)
      });
    },
    getDetail(index){
      console.log(index)
      let pids = this.shopArr[index].pid
      this.$router.push({path:"/shopDetail",query:{pid:pids}})
      // let shopDetailURL = this.$router.resolve({
      //   path:"/shopDetail",
      //   query:{
      //     pid:pids
      //   }
      // })
      // window.open(shopDetailURL.href,'_blank')
      // console.log(pids)
    }
  },
  created() {
